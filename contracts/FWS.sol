...
contract ProofSetService {
    ...
    function _onValidProof(uint256 ID) internal {
        ...
        if (proofsMissed > 0) {
            ...
            faults[ID].push(
                Faults(proofSets[ID].latestProofEpoch, block.number)
            );
        }
    ...
    // Read methods
    function getProofSetById(
        uint256 ID
    ) public view virtual returns (ProofSet memory) {
...
contract DealStoredOnchain is ProofSetService {
    ...
    function update(
        ...
        bytes[] calldata newDealsSignature
    ) public virtual {
        require(
            currentFaults(proofSetID) == 0,
            "Proof set is faulty, can not be updated"
        );
        uint proofSetLength = dealsMap[proofSetID].length;

        for (uint i = 0; i < removeDeals.length; i++) {
            require(
                removeDeals[i] < proofSetLength,
                "Deal ID is out of bounds"
            );

            uint256 escrowID = removeDeals[i];
            ProofSetIndex memory psIndex = dealsIndex[escrowID];
            require(
                psIndex.proofSetID == proofSetID,
                "Deal is not in the same proof set"
            );
            ...
            if (i < newDeals.length) {
                ...
                uint256 newEscrowID = Escrow(escrowAddress).start(
                    newDeals[i],
                    newDealsSignature[i]
                );
        ...
        for (uint i = removeDeals.length; i < newDeals.length; i++) {
            ...
            uint256 escrowID = Escrow(escrowAddress).start(
                newDeals[i],
                newDealsSignature[i]
            );
